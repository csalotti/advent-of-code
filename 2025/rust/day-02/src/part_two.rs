...
fn get_invalid(range: Range) -> Vec<u64> {
    ...
    'range: for val in range.0..=range.1 {
        let val_str = val.to_string();
        let val_length = val_str.chars().count();
        'windowing: for wsize in 1..=(val_length / 2) {
            let prefix = &val_str[..wsize];
            for j in (wsize..val_length).step_by(wsize) {
                if &val_str[j..(j + wsize).min(val_length)] != prefix {
                    continue 'windowing;
                }
            }
            invalid.push(val);
