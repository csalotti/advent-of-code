...
pub fn process(input: &'static str) -> u64 {
    let (freshness_ranges, ingredients_ids) = input.split_once("\n\n").unwrap();
    let freshness_ranges_merged = process_ranges(freshness_ranges);
    println!("Ranges : {freshness_ranges_merged:?}");
    let mut fresh_count = 0;
    'ingedient: for id in ingredients_ids
        .trim()
        .split("\n")
        .map(|i| i.trim().parse::<u64>().unwrap())
    {
        for range in freshness_ranges_merged.iter() {
            if range.is_in(id) {
                fresh_count += 1;
                continue 'ingedient;
            }
